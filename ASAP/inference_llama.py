...
class LlamaVanilla(Llama):
    def __init__(self, config):
        Llama.__init__(self, config)

    def compose_prompt(self, msg_system, msg_user_instruction):
        prompt = """[INST] <<SYS>>\n@msg_system\n<</SYS>>\n\n@msg_user_instruction [/INST]""" # omit <s> here because llama tokenizer prepend <s> token by default when tokenizing
        prompt = self.sub('@msg_system', msg_system, prompt)
        prompt = self.sub('@msg_user_instruction', msg_user_instruction, prompt)
        return prompt
...
class LlamaMTS(Llama):
    def __init__(self, config):
        Llama.__init__(self, config)
    
    def compose_prompt_retrieval(self, msg_system, msg_user_retrieval):
        prompt = """<s>[INST] <<SYS>>\n@msg_system\n<</SYS>>\n\n@msg_user_retrieval [/INST]"""
        prompt = self.sub('@msg_system', msg_system, prompt)
        prompt = self.sub('@msg_user_retrieval', msg_user_retrieval, prompt)
        return prompt
    
    def compose_prompt_score(self, msg_system, msg_user_retrieval, msg_assistant_retrieval, msg_user_score):
        prompt = """<s>[INST] <<SYS>>\n@msg_system\n<</SYS>>\n\n@msg_user_retrieval [/INST] @msg_assistant_retrieval </s></s>[INST] @msg_user_score [/INST]"""
        prompt = self.sub('@msg_system', msg_system, prompt)
        prompt = self.sub('@msg_user_retrieval', msg_user_retrieval, prompt)
        prompt = self.sub('@msg_assistant_retrieval', msg_assistant_retrieval, prompt)
        prompt = self.sub('@msg_user_score', msg_user_score, prompt)
        return prompt
...
def main():
    parser = argparse.ArgumentParser(description="inference llama2")
    ...
    dataset_path = f"./ASAP/resource/df_test_{args.method}.xlsx"
    ...
    class CONFIG:
        ...
        save_path = f"./ASAP/resource/df_test_{args.model_name}_{args.method}.xlsx"
    ...
    if args.method == 'vanilla':
        generator = LlamaVanilla(config)
    elif args.method == 'mts':
        generator = LlamaMTS(config)
